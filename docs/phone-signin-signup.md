...
## Matrix 外观的 Casdoor 托管页

统一手机号模式采用 Matrix 当前登录页的渐变网格背景、448px 玻璃卡片、品牌图标、分段标签和独立发码按钮；小屏输入框与按钮为 44px，支持深色主题及减少动态效果偏好。样式限定在启用应用的 `.matrix-auth` 内，不改变默认关闭应用的页面。

品牌使用 Casdoor 应用的 `displayName`（空时用 `name`）及 `logo`；未配置 Logo 时显示方块图标。页脚沿用现有 `footerHtml`，不把 Matrix 的业务品牌查询接入身份服务。当前采用统一居中布局，不使用旧 `formOffset`、侧图和表单外层自定义 CSS；完全自定义的 `signinHtml` 仍优先。上线前需检查目标应用是否依赖这些旧布局配置。

认证表单继续使用 Casdoor 原组件，保留语言、第三方入口、MFA、验证码与授权链路。用户可见的差异是：协议需要显式勾选、国际区号仍可选择、增长邀请码留在 Matrix。后续由 Matrix 和子应用发起 OIDC 跳转进入此页面，不能仅用后端代理请求代替浏览器在 Casdoor 建立会话。
