...
func CreateHduBindingTicket(user *User, returnURL, callbackOrigin string) (string, error) {
	bytes := make([]byte, 32)
...
func GetActiveHduBindingTicket(token string) (*HduBindingTicket, error) {
	...
	if err != nil || !exists || record.ExpiresAt < time.Now().Unix() {
		return nil, err
	}
	return record, nil
}
...
func ConsumeHduBindingTicket(token string) (bool, error) {
	deleted, err := ormer.Engine.
		Where("token_hash = ? AND expires_at >= ?", hduBindingTokenHash(token), time.Now().Unix()).
		Delete(&HduBindingTicket{})
